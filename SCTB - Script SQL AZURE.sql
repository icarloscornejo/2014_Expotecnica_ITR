...
dui_datpersona varchar(300),
nit_datpersona varchar(300),
...
usu_usuario varchar(300),
contra_usuario varchar(300),
id_persona int,
foreign key (id_persona) references tbl.Personas(id_persona) on delete cascade on update cascade,
...
alter table tbl.Personas
add id_usuario int,
foreign key (id_usuario) references tbl.Usuarios(id_usuario)
GO
...
num_cuenta varchar(300),
pin_cuenta varchar(300),
id_persona int,
id_tipcuenta int,
foreign key (id_persona) references tbl.Personas(id_persona) on delete cascade on update cascade,
...
id_bitestprestamo int not null identity primary key,
id_prestamo int,
id_estprestamo int,
...
select per.nom_persona, per.ape_persona , pr.monto_prestamo, pr.cuota_prestamo, tp.nom_tipprestamo, pl.nom_plazo, pl.cuotasmensuales_plazo, ep.nom_estprestamo, c.id_cuenta
from tbl.Prestamos as pr, tbl.bitEstadosdePrestamos as bep, tbl.EstadosdePrestamos ep, tbl.TiposdePrestamos as tp, tbl.Plazos as pl, tbl.Cuentas as c, tbl.Personas as per
where tp.id_tipprestamo = pr.id_tipprestamo and pl.id_plazo = pr.id_plazo and pr.id_prestamo = bep.id_prestamo and ep.id_estprestamo = bep.id_estprestamo and c.id_cuenta = pr.id_cuenta and per.id_persona = c.id_persona
GO
...
create view viw.Cliente as
select p.nom_persona, p.ape_persona, p.correo_persona, dp.dui_datpersona, dp.nit_datpersona, dp.telcasa_datpersona, g.nom_genero, ec.nom_estadocivil, t.num_tarjeta, c.num_cuenta, c.id_cuenta
from tbl.Personas as p, tbl.DatosdePersonas as dp, tbl.Generos as g, tbl.EstadosCiviles as ec, tbl.Cuentas as c, tbl.Tarjetas as t
where p.id_persona = c.id_persona AND p.id_persona = dp.id_persona AND g.id_genero = dp.id_genero AND ec.id_estadocivil = dp.id_estadocivil AND c.id_cuenta = t.id_cuenta
GO

select * from viw.Cliente
...
CREATE PROC pcd.AgregarPersona(@nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @nom_sucursal varchar(50), @nom_tippersona varchar(50), @id_usuario int, @id_persona int OUTPUT)
AS
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO tbl.Personas VALUES(@nom_persona, @ape_persona, @correo_persona,GETDATE(), @id_tippersona, @id_usuario)
...
CREATE PROC pcd.ModificarPersona(@id_persona int, @nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(100))
AS
BEGIN
...
CREATE PROC pcd.AgregarPersonaADM(@nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @nom_tippersona varchar(50), @id_usuario int, @id_persona int OUTPUT)
AS
BEGIN
...
CREATE PROC pcd.AgregarPersonaconDatos(@nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @nom_sucursal varchar(50), @nom_tippersona varchar(50), @id_usuario int, @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50))
AS
BEGIN
	...
	BEGIN TRY
		...
		EXEC pcd.AgregarPersona @nom_persona, @ape_persona, @correo_persona, @nom_sucursal, @nom_tippersona, @id_usuario, @id_persona = @id OUTPUT
		INSERT INTO tbl.DatosdePersonas VALUES(@dui_datpersona, @nit_datpersona, @dir_datpersona, @telcel_datpersona, @telcasa_datpersona, @id_genero, @id_estcivil, @id)
...
CREATE PROC pcd.ModificarPersonaconDatos(@id_persona int, @nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50))
AS
BEGIN
	...
	BEGIN TRY
		...
		SET @id_genero = (SELECT id_genero FROM tbl.Generos WHERE nom_genero = @nom_genero)
		SET @id_estcivil = (SELECT id_estadocivil FROM tbl.EstadosCiviles WHERE nom_estadocivil = @nom_estcivil)
		EXEC pcd.ModificarPersona @id_persona, @nom_persona, @ape_persona, @correo_persona
		UPDATE tbl.DatosdePersonas SET dui_datpersona = @dui_datpersona,nit_datpersona = @nit_datpersona,dir_datpersona = @dir_datpersona,telcel_datpersona = @telcel_datpersona,telcasa_datpersona = @telcasa_datpersona,id_genero = @id_genero,id_estadocivil = @id_estcivil WHERE id_persona = @id_persona
...
CREATE PROC pcd.ModificarContraseña(@id_usuario int, @nueva_contraseña varchar(300))
AS
BEGIN
	BEGIN TRANSACTION ModificarContraseña 
	BEGIN TRY
		UPDATE tbl.Usuarios SET contra_usuario = @nueva_contraseña WHERE id_usuario = @id_usuario
		COMMIT TRAN ModificarContraseña
		RETURN
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN ModificarContraseña
		DECLARE @Mensaje NVARCHAR(4000) = 'Error al intentar modificar a una contraseña.';
		DECLARE @Severidad INT = ERROR_SEVERITY();
		DECLARE @Estado INT = ERROR_STATE();
		RAISERROR (@Mensaje, @Severidad, @Estado) ;
	END CATCH
...
CREATE PROC pcd.AgregarSucursal(@nom_sucursal varchar(50), @dir_sucursal varchar(50), @nom_departamento varchar(50))
AS
BEGIN
	BEGIN TRANSACTION AgregarSucursal 
	BEGIN TRY
		DECLARE @id_departamento int
		SET @id_departamento = (SELECT id_departamento FROM tbl.Departamentos WHERE nom_departamento = @nom_departamento)
		INSERT INTO tbl.Sucursales VALUES(@nom_sucursal, @dir_sucursal, @id_departamento)
		COMMIT TRAN AgregarSucursal
		RETURN
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN AgregarSucursal
		DECLARE @Mensaje NVARCHAR(4000) = 'Error al intentar agregar a una sucursal.';
		DECLARE @Severidad INT = ERROR_SEVERITY();
		DECLARE @Estado INT = ERROR_STATE();
		RAISERROR (@Mensaje, @Severidad, @Estado) ;
	END CATCH
...
CREATE PROC pcd.ModificarSucursal(@temp_nomsucursal varchar(50), @nom_sucursal varchar(50), @dir_sucursal varchar(50), @nom_departamento varchar(50))
AS
BEGIN
	BEGIN TRANSACTION ModificarSucursal 
	BEGIN TRY
		DECLARE @id_sucursal int
		DECLARE @id_departamento int
		SET @id_sucursal = (SELECT id_sucursal FROM tbl.Sucursales WHERE nom_sucursal = @temp_nomsucursal)
		SET @id_departamento = (SELECT id_departamento FROM tbl.Departamentos WHERE nom_departamento = @nom_departamento)
		UPDATE tbl.Sucursales SET nom_sucursal = @nom_sucursal, dir_sucursal = @dir_sucursal, id_departamento = @id_departamento WHERE id_sucursal = @id_sucursal
		COMMIT TRAN ModificarSucursal
		RETURN
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN ModificarSucursal
		DECLARE @Mensaje NVARCHAR(4000) = 'Error al intentar modificar a una sucursal.';
		DECLARE @Severidad INT = ERROR_SEVERITY();
		DECLARE @Estado INT = ERROR_STATE();
		RAISERROR (@Mensaje, @Severidad, @Estado) ;
	END CATCH
...
CREATE PROC pcd.EliminarSucursal(@nom_sucursal varchar(50))
AS
BEGIN
	BEGIN TRANSACTION EliminarSucursal 
	BEGIN TRY
		DECLARE @id_sucursal int
		SET @id_sucursal = (SELECT id_sucursal FROM tbl.Sucursales WHERE nom_sucursal = @nom_sucursal)
		DELETE FROM tbl.Sucursales WHERE id_sucursal = @id_sucursal
		COMMIT TRAN EliminarSucursal
		RETURN
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN EliminarSucursal
		DECLARE @Mensaje NVARCHAR(4000) = 'Error al intentar eliminar a una sucursal.';
		DECLARE @Severidad INT = ERROR_SEVERITY();
		DECLARE @Estado INT = ERROR_STATE();
		RAISERROR (@Mensaje, @Severidad, @Estado) ;
	END CATCH
...
CREATE PROC pcd.AgregarCliente(@nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @id_usuario int, @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50), @salario_dattrabajo money, @nom_datempresa varchar(100), @nom_cargotrabajo varchar(50))
AS
BEGIN
	...
	BEGIN TRY
		...
		EXEC pcd.AgregarPersonaconDatos @nom_persona, @ape_persona, @correo_persona, 'Bancaria Central', 'Cliente', @id_usuario, @dui_datpersona, @nit_datpersona, @dir_datpersona, @telcel_datpersona, @telcasa_datpersona, @nom_genero, @nom_estcivil
...
CREATE PROC pcd.ModificarCliente(@id_persona int, @nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50), @salario_dattrabajo money, @nom_datempresa varchar(100), @nom_cargotrabajo varchar(50))
AS
BEGIN
	...
	BEGIN TRY
		...
		SET @id_datempresa = (SELECT id_datempresa FROM tbl.DatosdeEmpresas WHERE nom_datempresa = @nom_datempresa)
		SET @id_cargotrabajo = (SELECT id_cargotrabajo FROM tbl.CargosdeTrabajos WHERE nom_cargotrabajo = @nom_cargotrabajo)
		EXEC pcd.ModificarPersonaconDatos  @id_persona, @nom_persona, @ape_persona, @correo_persona, @dui_datpersona, @nit_datpersona, @dir_datpersona, @telcel_datpersona, @telcasa_datpersona, @nom_genero, @nom_estcivil
...
CREATE PROC pcd.AgregarEmpleado(@nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @nom_sucursal varchar(50), @id_usuario int, @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50), @usu_usuario varchar(300), @contra_usuario varchar(300))
AS
BEGIN
	BEGIN TRANSACTION AgregarEmpleado
	BEGIN TRY
		DECLARE @id int
		EXEC pcd.AgregarPersonaconDatos @nom_persona, @ape_persona, @correo_persona, @nom_sucursal, 'Empleado', @id_usuario, @dui_datpersona, @nit_datpersona, @dir_datpersona, @telcel_datpersona, @telcasa_datpersona, @nom_genero, @nom_estcivil
		SET @id = (SELECT id_persona FROM tbl.Personas WHERE nom_persona = @nom_persona AND ape_persona = @ape_persona AND correo_persona = @correo_persona)
		INSERT INTO tbl.Usuarios VALUES(@usu_usuario, @contra_usuario, @id)
...
CREATE PROC pcd.ModificarEmpleado(@id_persona int, @nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @nom_sucursal varchar(50), @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50), @usu_usuario varchar(300))
AS
BEGIN
	...
	BEGIN TRY
		...
		SET @id_sucursal = (SELECT id_sucursal FROM tbl.Sucursales WHERE nom_sucursal = @nom_sucursal)
		EXEC pcd.ModificarPersonaconDatos @id_persona, @nom_persona, @ape_persona, @correo_persona, @dui_datpersona, @nit_datpersona, @dir_datpersona, @telcel_datpersona, @telcasa_datpersona, @nom_genero, @nom_estcivil
		UPDATE tbl.bitSucursales SET id_sucursal = @id_sucursal WHERE id_persona = @id_persona
		UPDATE tbl.Usuarios SET usu_usuario = @usu_usuario WHERE id_persona = @id_persona
...
CREATE PROC pcd.AgregarGerente(@nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @nom_sucursal varchar(50), @id_usuario int, @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50), @usu_usuario varchar(300), @contra_usuario varchar(300))
AS
BEGIN
	BEGIN TRANSACTION AgregarGerente
	BEGIN TRY
		DECLARE @id int
		EXEC pcd.AgregarPersonaconDatos @nom_persona, @ape_persona, @correo_persona, @nom_sucursal, 'Gerente', @id_usuario, @dui_datpersona, @nit_datpersona, @dir_datpersona, @telcel_datpersona, @telcasa_datpersona, @nom_genero, @nom_estcivil
		SET @id = (SELECT id_persona FROM tbl.Personas WHERE nom_persona = @nom_persona AND ape_persona = @ape_persona AND correo_persona = @correo_persona)
		INSERT INTO tbl.Usuarios VALUES(@usu_usuario, @contra_usuario, @id)
...
CREATE PROC pcd.ModificarGerente(@id_persona int, @nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50), @nom_sucursal varchar(50), @dui_datpersona varchar(300), @nit_datpersona varchar(300), @dir_datpersona varchar(100), @telcel_datpersona varchar(9), @telcasa_datpersona varchar(9), @nom_genero varchar(50), @nom_estcivil varchar(50), @usu_usuario varchar(300))
AS
BEGIN
...
CREATE PROC pcd.AgregarAdministrador(@nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50),  @usu_usuario varchar(300), @contra_usuario varchar(300), @id_usuario int)
AS
BEGIN
	...
	BEGIN TRY
		...
		IF NOT EXISTS(SELECT id_persona FROM tbl.Personas WHERE correo_persona = @correo_persona)
			BEGIN
			EXEC pcd.AgregarPersonaADM @nom_persona, @ape_persona, @correo_persona, 'Administrador', @id_usuario , @id_persona = @id OUTPUT
			SET @id = (SELECT id_persona FROM tbl.Personas WHERE nom_persona = @nom_persona AND ape_persona = @ape_persona AND correo_persona = @correo_persona)
			SET @verificador = 0
			END
		ELSE
			...
			SET @verificador = 1
			END
		IF NOT EXISTS(SELECT id_usuario FROM tbl.Usuarios WHERE usu_usuario = @usu_usuario)
			BEGIN			
			INSERT INTO tbl.Usuarios VALUES(@usu_usuario, @contra_usuario, @id)
			END
		ELSE
		...
		IF(@verificador = 1)
			BEGIN
			RAISERROR ('Persona o usuario ya existente.', 19, 503) ;
			END
...
CREATE PROC pcd.ModificarAdministrador(@id_persona int, @nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50),  @usu_usuario varchar(300))
AS
BEGIN
...
BEGIN
	...
	BEGIN TRY
		DECLARE @id int
		DECLARE @id_tippersona int
		SET @id_tippersona = (SELECT id_tippersona FROM tbl.TiposdePersonas WHERE nom_tippersona = 'Superadministrador')
		INSERT INTO tbl.Personas(nom_persona, ape_persona, correo_persona, feccreacion_persona, id_tippersona) VALUES(@nom_persona, @ape_persona, @correo_persona,GETDATE(), @id_tippersona)
		SET @id = (SELECT id_persona FROM tbl.Personas WHERE nom_persona = @nom_persona AND ape_persona = @ape_persona AND correo_persona = @correo_persona)
		INSERT INTO tbl.Usuarios VALUES(@usu_usuario, @contra_usuario, @id)
...
CREATE PROC pcd.ModificarSuperAdministrador(@id_persona int, @nom_persona varchar(50), @ape_persona varchar(50), @correo_persona varchar(50),  @usu_usuario varchar(300), @contra_usuario varchar(300))
AS
BEGIN
	BEGIN TRANSACTION ModificarSuperAdministrador 
	BEGIN TRY
		EXEC pcd.ModificarPersona @id_persona, @nom_persona, @ape_persona, @correo_persona
		UPDATE tbl.Usuarios SET usu_usuario = @usu_usuario,contra_usuario = @contra_usuario WHERE id_persona = @id_persona
...
CREATE PROC pcd.NuevoPrestamo(@monto_prestamo money, @nom_tipprestamo varchar(50), @nom_plazo varchar(50), @nom_estprestamo varchar(50), @id_cuenta int)
AS
BEGIN
	...
	BEGIN TRY
		...
		SET @id_prestamo = (SELECT id_prestamo FROM tbl.Prestamos WHERE monto_prestamo = @monto_prestamo AND cuota_prestamo = @cuota_prestamo AND id_tipprestamo = @id_tipprestamo AND id_cuenta = @id_cuenta AND id_plazo = @id_plazo)
		INSERT INTO tbl.bitEstadosdePrestamos VALUES(@id_prestamo, @id_estprestamo)
...
BEGIN
	...
	BEGIN TRY
		...
		SET @id_cuenta = (SELECT id_cuenta FROM tbl.Cuentas WHERE num_cuenta = @num_cuenta)
		SET @id_estremesa = (SELECT id_estremesa FROM tbl.EstadosdeRemesas WHERE nom_estremesa = 'Enviada')
		INSERT INTO tbl.Remesas VALUES(@monto_remesa, @nomdestinatario_remesa, @pin_remesa, @id_cuenta)
...
BEGIN
	...
	BEGIN TRY
		...
		SET @id_remesa = (SELECT id_remesa FROM tbl.Remesas WHERE nomdestinatario_remesa = @nomdestinatario_remesa AND pin_remesa = @pin_remesa)
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO tbl.Cuentas VALUES(@num_cuenta, @pin_cuenta, @id_persona, @id_tipcuenta)
		...
		INSERT INTO tbl.Ahorros VALUES(0, @id_cuenta)
...
BEGIN
	...
	BEGIN TRY
		...
		SET @id_cuenta = (SELECT id_cuenta FROM tbl.Cuentas WHERE num_cuenta = @num_cuenta)
		SET @id_estcuenta = (SELECT id_estcuenta FROM tbl.EstadosdeCuentas WHERE nom_estcuenta = @nom_estcuenta)
		UPDATE tbl.Cuentas SET pin_cuenta = @pin_cuenta WHERE id_cuenta = @id_cuenta
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO tbl.Tarjetas VALUES(@num_tarjetadebito, @id_cuenta, @id_clatarjetadebito)
		INSERT INTO tbl.Tarjetas VALUES(@num_tarjetacredito, @id_cuenta, @id_clatarjetacredito)
		SET @id_tarjetadebito = (SELECT id_tarjeta FROM tbl.Tarjetas WHERE num_tarjeta = @num_tarjetadebito)
		SET @id_tarjetacredito = (SELECT id_tarjeta FROM tbl.Tarjetas WHERE num_tarjeta = @num_tarjetacredito)
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @id_esttarjeta int
		SET @id_tarjeta = (SELECT id_tarjeta FROM tbl.Tarjetas WHERE num_tarjeta = @num_tarjeta)
